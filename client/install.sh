...
# Redeem invite and download config
echo "Redeeming invite from lighthouse..."
curl -f "http://$LIGHTHOUSE_IP/client/redeem_invite?invite=$INVITE_CODE" -o nebula_config.zip
echo "Extracting nebula_config.zip..."
unzip -o nebula_config.zip

# Start nebula in background
echo "Starting nebula with config.yaml..."
nebula -config config.yaml &
NEBULA_PID=$!
echo "nebula started with PID $NEBULA_PID"
echo "Done."
